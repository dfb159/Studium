...
public class SozialesNetzwerk {
	...
	public static void main(String[] args) {
		...
		Person[] p = new Person[] { new Person("Theresa", new Person[] {}),
				new Person("Thomas", new Person[] {}),
				new Person("Kerstin", new Person[] {}),
				new Person("Adam", new Person[] {}),
				new Person("Michael", new Person[] {}), };
	...
	/**
	 * Die Methode sucht fuer jeden Freund eine Kette zu der geforderten Person.
	 * Schritt fuer Schritt wird solange die maximale Laenge der Kette
	 * heruntergesetzt, bis keine Kette mehr gefunden wird. Dies garantiert die
	 * kuerzeste Freundschaftskette.
	 * 
	 * @param start
	 *            Die Startperson der Kette
	 * @param ende
	 *            Die Endperson der Kette
	 * @return Die vollstaendige Kette der beiden Personen mit einer maximalen
	 *         Laenge von 6
	 */
	...
	/**
	 * Diese Untermethode sucht rekursiv fuer die angegebene Startperson einen
	 * Weg zur Zielperson. Falls die maximale Kettenlaenge oder die Zielperson
	 * erreicht ist, wird rueckwirkend eine Kette der Personen aufgebaut.
	 * 
	 * @param p
	 *            Die Zielkette
	 * @param iteration
	 ...
	 * @param current
	 *            Die momentane Person, welche getestet werden soll, ob sie eine
	 *            Verbindungskette aufweist.
	 * @param ende
	 *            Die Zielperson
	 * @param maxIterations
	 ...
	 */
	private boolean recursiveFreundschaftskette(Person[] p, int iteration,
			Person current, Person ende, int maxIterations) {
		if (iteration >= maxIterations)
			return false;
		if (current.equals(ende))
			return true;
		for (Person freund : current.getFreunde()) {
			if (recursiveFreundschaftskette(p, iteration + 1, freund, ende,
					maxIterations)) {
				p[iteration] = freund;
				return true;
			}
	...
	/**
	 * Die Methode gibt die tatsaechliche Laenge der Kette wieder. Dabei werden
	 * alle null Werte am Ende der Kette ignoriert.
	 * 
	 * @param kette

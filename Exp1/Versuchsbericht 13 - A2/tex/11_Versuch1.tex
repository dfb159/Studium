\section{Methoden}
	...
	\subsection{Aufbau}		
		...
		Aus der Kathode werden Elektronen rausgelöst und zur Anode geschickt.
		...
		Diese wird über Operationsverstärker im Betriebsgerät verstärkt wird und ist einfacher zu messen.
	...
	\subsection{Unsicherheiten}
		...
		Jegliche Unsicherheiten werden nach GUM bestimmt und berechnet\footnote{Die Gleichungen dazu finden sich im Anhang unter ... }. %TODO GUM-Formeln im Anhang verlinken.
		...
		Die Beschleunigungsspannung wird analog eingestellt und wird mit einem digitalen Multimeter gemessen ($\Delta U = \SI{0.01}{\Volt}$).
...
\section{Durchführung und Datenanalyse}
	...
	\end{align}
	Dabei dient der Siedepunkt von Quecksilber als Referenzpunkt\site{boilingPoint}\footnote{Wie in der Quelle unter dem Punkt Conclusions angemerkt, kann den beiden Werten eine relative Unsicherheit von $1\%$ zugeordnet werden.}.
	Die Werte sind $T_1 = \SI{629.77}{K}s$, sowie $p_1 = \SI{101.325}{\kilo\Pascal}$.
	$R = \SI{8.3145}{\Joule\per\Mol\per\Kelvin}$ ist die allgemeine Gaskonstante\site{Constants} und $H_\texttt{m,v} = \SI{59.3+-0.1}{\kilo\Joule\per\Mol}$ die molare Verdamfpungsenthalpie bzw. spezifische Verdampfungswärme\site{verdampfungswaerme}.
	...
	\end{align}
 	Einsetzen mit $\sigma = \pi r_e^2$\site{Constants} ergibt bei Raumtemperatur $\lambda_\text{frei, Raumtemperatur} = \SI{8.4+-1.5}{10^8\meter}$ und erhitzt $\lambda_\text{frei, Ofentemperatur} = \SI{1.40+-0.15}{10^6\meter}$. 
...
\section{Diskussion}
	...
	Dadurch bildet sich stattdessen die für eine Triode typische Charakteristikenkurve aus, welche einen ungefähren logarithmischen Verlauf hat.
	
	Entgegen den Erwartungen jedoch liegen die berechneten Werte der freien Weglänge deutlich über den erwarteten Werten.
	...
	Während die Weglänge bei Ofentemperatur immerhin niedriger ist, als die bei Raumtemeratur, so ist sie immer noch viel größer als der Abstand Kathode-Gitter.
